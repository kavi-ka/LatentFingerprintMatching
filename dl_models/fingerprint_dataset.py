...
class FingerprintDataset(Dataset):
    ...
    def load_images(self, root_dirs):
        ...
        self.classes_latent = list()
        self.img_labels_latent = list()
        self.images_latent = list()
        self.class_to_images_latent = list()

        if type(root_dirs) is not list:
            root_dirs = [root_dirs]
        # can have multiple data sources
        for root_dir in root_dirs:
            if "latent" in root_dir:
                # go through all people
                for pid in os.listdir(root_dir):
                    # classes are people
                    self.classes_latent.append(pid)
                    # persons to their images
                    self.class_to_images_latent.append(list())
                    ...
                    for sample in os.listdir(curr_person_folder):
                        ...
                        self.img_labels_latent.append(pid)
                        self.images_latent.append(curr_image)
                        self.class_to_images_latent[-1].append(curr_image)
            else:
                # go through all people
                for pid in os.listdir(root_dir):
                    # classes are people
        ...
        self.len_latent = len(self.img_labels_latent)

        print("ingested data of non-latent:", self.len, "latent:", self.len_latent)
    ...
    def __init__(self, root_dir, train=True):
        ...
        if self.train:
            ...
            # the person id
            self.train_labels_latent = self.img_labels_latent
            # the file paths
            self.train_data_latent = self.images_latent
        else: # test
            self.test_labels = self.img_labels
            self.test_data = self.images

            # the person id
            self.test_labels_latent = self.img_labels_latent
            # the file paths
            self.test_data_latent = self.images_latent
